        ...
        block.update(
            {
                ...
                "vol_windows": [
                    int(w) for w in (windows.get("volatility", [20]) or [20]) if int(w) > 1
                ],
...
def _normalize_features_config(features: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        "multicollinearity_threshold" not in features
        and "multicollinearity_threshold" in generation
    ):
    ...
    for family in ("technical", "context", "fundamentals", "sentiment"):
        ...
        preset_name = (
            family_choice.get("preset")
            or family_def.get("default_preset")
            or next(iter((family_def.get("presets", {}) or {"default": {}}).keys()))
        )
